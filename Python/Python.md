...
`self` 参数是对类的当前实例的引用，用于访问属于该类的变量，不要求被命名为self，但是必须为收个参数
``` Python
class Student:
    # 初始化属性
...
# 名词解释
标识符：函数，类，对象等编程实体的名称
保留关键字：含有特殊意义的字符，不能用于标识符
数据类型：表示数据的类别，它们定义了数据的大小、存储方式和可执行的操作。
解释器：**逐行**解释执行源代码，将代码逐行翻译成机器指令并立即执行。
编译器：将源代码**整体**翻译成机器码或字节码，然后再执行翻译后的代码。
运算符：用于执行程序代码运算的符号。
字符串：存储字符的一种数据容器。
列表：内置的一种数据结构
元组：一种不可修改的数据容器
字典：内置的数据结构，它以键值对的形式存储数据。
集合：是一个无序的不重复元素序列数据结构
条件控制：是通过一条或多条语句的执行结果（True 或者 False）来决定是否执行。
迭代器：是一个可以记住遍历的位置的对象，从集合的第一个元素开始访问，直到所有的元素被访问完结束。迭代器只能往前不会后退。
生成器：用于定义生成器函数，生成器函数是一种特殊的函数，可以在迭代过程中逐步产生值，而不是一次性返回所有结果。
类：类似对象构造函数，或者用于创建对象的蓝图
对象：一个应用系统中用来描述客观事物的实体
多态：在许多对象或类上执行的具有相同名称的方法/函数/操作符。