...
路由与转发：路由则是路由器相互配合规划IP数据报转发途径，生成路由表；转发则根据自己的转发表将收到的数据报从合适的接口转发出去。转发表=精简路由表（only self）
...
2后断开的是数据通道，控制信号仍可发送，挥手2、3可以无限接近于0，所以可认为2、3次挥手是同时发生的，从第一次挥手开始到服务器关闭连接，可认为仅使用1.5倍RTT时间
