...
# 汇编与反汇编
编程语言为高级语言
汇编语言是低级语言，用一些**助记符**表示计算机的指令

**计算机程序最终都是机器码文件**，反汇编则是转译回汇编语言
通过反汇编器将机器码还原为汇编代码
编译器生成后的优化代码会破坏程序逻辑的**直观性**，增加反汇编后的理解难度

反汇编器：IDA Pro、Ghidra等
调试器：WinDbg（系统级调试）、x64dbg（动态分析）
反编译器：尝试还原回高级语言Hex-Rays

## 逆向
预处理 编译.c 链接.obj 运行.exe
Linux下为.o
文件遵循特定格式，Windows下为COFF，Linux下为ELF

编译：将高级语言转换为汇编语言或目标机器语言
链接：多个目标文件以及所需要的库文件组成一个可执行文件，连接器主要任务是解析目标文件中的外部符号引用，将它们与定义这些符号的目标文件或库文件进行匹配和连接
...
## 虚拟化保护
程序运行在VM中时，所有的指令都会经过VM加壳进行转码。进行逆向分析时需要分析加密过程，翻译回正常指令

# IDA Pro
选取后按X可查看交叉引用
-号可查看调用层级，和编译器单步调试时的调用堆栈类似

# 未分类
main函数完整原型：
int main(int argc, const char** argv,  const char** envp);
argc 参数数量argument count，含程序本身路径，永远>=1
argv 参数数组argument vector，每个元素都是一个字符串
envp 环境变量数组enviroment pointer，每个元素都是一个环境变量的字符串 